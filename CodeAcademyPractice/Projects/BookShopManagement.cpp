...
class employees
{
        ...
        long int salary;
        string mgr_status; //check(T of F) def f
    public:
        void add_emp();
        void search_emp();
...
void books::update_price()
{
        ...
        cin >> id;
        stmt.str("");
        stmt << "Select name, price form books where id = " << id << ";";
        query = stmt.str();
        q = query.c_str();
        mysql_query(conn, q);
        res_set = mysql_store_result(conn);
        if((row = mysql_fetch_row(res_set)) != NULL)
        {
            ...
            cin >> choice;
                    if (choice == 121 || choice == 89)
                    {
                            ...
                            cout << endl << endl << "Entry Error !" << endl << "Contact Technical Team " << endl 
                    }
                    else
                    {
...
void books::search()
{
        cout << "Enter books id for details : ";
        cin >> id;
    stmt.str("");
...
void employees::add_emp()
{
        ...
        cout << "Enter The name of the empoyee : " << endl;
        cin >> name;
        cout << "Enter street Address (in 3 lines) : " << endl;
        cin >> addr_line1;
        cin >> addr_line2;
        cin >> addr_city;
        cout << "Enter State : ";
...
void members::add__mem()
{
        ...
        cin >> addr_line2;
        cin >> addr_city;
        cout << "Enter the name of thw state : ";
        cin >> addr_state;
        stmt.str("");
        stmt << "insert into memmbers(name,addr1,addr2,addr_city,addr_stat,phone_no,beg_date,end_date) values ('" << name << "','" << addr_line1 << "','" << addr_line2 << "','" << addr_city << "','" << addr_state << "'," << phn << ",curedate(),Date_add(curdate(), INTERVAL 1 YEAR));";
        query = stmt.str();
        q = query.c_str();
        mysql_query(conn,q);
        //fetching member id...
